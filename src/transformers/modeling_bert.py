...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def mms_loss(self, logit, mode='mean', optimize=True):
        topk_vals, pred_classes = logit.topk(2, 1)

        # with torch.no_grad():
        w1 = self.classifier.weight.index_select(0, pred_classes[:, 0])
        w2 = self.classifier.weight.index_select(0, pred_classes[:, 1])
    ...
    def max_radious(self, features, optimize=True):
    ...
    def forward(
        ...
        radius = self.max_radious(pooled_output, optimize=True)
        mms_scaled = radius / self.mms_loss(logits, optimize=True)
        # mms_scaled = -torch.log(torch.sigmoid(self.mms_loss(logits, optimize=True) / radius)).mean()
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                loss_fct = CrossEntropyLoss()
                loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1)) + 0.01 * mms_scaled
